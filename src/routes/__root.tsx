import { Outlet, Link, createRootRoute, HeadContent, Scripts } from "@tanstack/react-router";

import appCss from "../styles.css?url";

function NotFoundComponent() {
  return (
    <div className="flex min-h-screen items-center justify-center bg-background px-4">
      <div className="max-w-md text-center">
        <h1 className="text-7xl font-bold text-foreground">404</h1>
        <h2 className="mt-4 text-xl font-semibold text-foreground">Page not found</h2>
        <p className="mt-2 text-sm text-muted-foreground">
          The page you're looking for doesn't exist or has been moved.
        </p>
        <div className="mt-6">
          <Link
            to="/"
            className="inline-flex items-center justify-center rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground transition-colors hover:bg-primary/90"
          >
            Go home
          </Link>
        </div>
      </div>
    </div>
  );
}

export const Route = createRootRoute({
  head: () => ({
    meta: [
      { charSet: "utf-8" },
      { name: "viewport", content: "width=device-width, initial-scale=1" },
      { title: "Align Care — Mental Health & Recovery Matching" },
      { name: "description", content: "Align Care matches you with mental health and substance use treatment based on your clinical needs. Confidential and compassionate." },
      { name: "author", content: "Align Care" },
      { property: "og:title", content: "Align Care — Mental Health & Recovery Matching" },
      { property: "og:description", content: "Confidential clinical matching for mental health and substance use treatment." },
      { property: "og:type", content: "website" },
      { name: "twitter:card", content: "summary" },
      { name: "twitter:title", content: "Align Care — Mental Health & Recovery Matching" },
      { name: "twitter:description", content: "Confidential clinical matching for mental health and substance use treatment." },
      { property: "og:image", content: "https://pub-bb2e103a32db4e198524a2e9ed8f35b4.r2.dev/f0149928-bc45-4bd8-877e-833f72f0e06a/id-preview-4575aa5d--c8d40fe5-1e98-4771-a145-fb63988a11e9.lovable.app-1776528221728.png" },
      { name: "twitter:image", content: "https://pub-bb2e103a32db4e198524a2e9ed8f35b4.r2.dev/f0149928-bc45-4bd8-877e-833f72f0e06a/id-preview-4575aa5d--c8d40fe5-1e98-4771-a145-fb63988a11e9.lovable.app-1776528221728.png" },
    ],
    links: [
      {
        rel: "stylesheet",
        href: appCss,
      },
    ],
  }),
  shellComponent: RootShell,
  component: RootComponent,
  notFoundComponent: NotFoundComponent,
});

function RootShell({ children }: { children: React.ReactNode }) {
  return (
    <html lang="en">
      <head>
        <HeadContent />
      </head>
      <body>
        {children}
        <Scripts />
      </body>
    </html>
  );
}

function RootComponent() {
  return <Outlet />;
}
